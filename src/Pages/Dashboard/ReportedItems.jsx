...
const ReportedItems = () => {
    ...
    const { data: reportItem = [], refetch } = useQuery({
        queryKey: ['item'],
        queryFn: async () => {
            const res = await fetch(`https://phone-garage-server-xi.vercel.app/items`)
                                        ...
                                        <>
                                            <tr key={user._id} className="border-b border-opacity-20 dark:border-gray-700 dark:bg-gray-900">
                                                ...
                                                <td className="p-3 text-left">
                                                    <label onClick={() => setDeleteItem(user)} htmlFor="confirmation-modal" className="btn btn-sm btn-error cursor-pointer">Delete</label>
                                                </td>
                                            </tr>
