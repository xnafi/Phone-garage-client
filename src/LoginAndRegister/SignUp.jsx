...
const SignUp = () => {
    ...
    const handleGoogleSignIn = () => {
        signInWithGoogle()
            .then(res => {
                const newUdata = res?.user
                const newUser = {
                    name: newUdata.displayName,
                    email: newUdata.email,
                    image: newUdata.photoURL,
                    role: 'buyer',
                    isAdmin: false
                }
                axios.post('https://phone-garage-server-xi.vercel.app/users', newUser)
            ...
            <div className="w-full max-w-md md:p-8 p-2 space-y-3 rounded-xl dark:bg-gray-900 dark:text-gray-100 bg-neutral">
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate="" action="" className="space-y-6 ng-untouched ng-pristine ng-valid">
                    <div className="space-y-1 text-sm">
                        ...
                        <input type="text"  {...register("name", { required: true })} id="username" placeholder="Username" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
                    </div>
                    <div className="space-y-1 text-sm">
                        <label for="email" className="block dark:text-gray-400">Email</label>
                        <input type="email"  {...register("email", { required: true })} id="email" placeholder="Enter your email" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
                    </div>
                    <div className="space-y-1 text-sm">
                        <label for="image" className="block dark:text-gray-400">Image</label>
                    ...
                    <div className="dropdown w-full space-y-1 text-sm">
                        <label for="role" className="block dark:text-gray-400">Select Role</label>
                        <select {...register("selectOption", { required: true })} className="select w-full bg-white">
                            {
                                option.map(sp => <option key={sp._id}>{sp.name}</option>)
                    ...
                    <button type='submit' className="block w-full p-3 text-center rounded-sm dark:text-gray-900 dark:bg-violet-400 btn glass">Sign up</button>
                </form>
